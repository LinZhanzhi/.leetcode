...
public:
                    ...
                    function<void()> putRightFork)
    {
        ...
        int right = (philosopher + 1) % 5; // right fork index

        // Asymmetric locking to avoid circular wait:
        // philosophers 0–3 take left then right,
        // philosopher 4 takes right then left.
        if (philosopher == 4)
        {
        ...
        // Put down forks and unlock in reverse order
        if (philosopher == 4)
        {
            putLeftFork();
            forks[left].unlock();
