...
public:
    int countMonobit(int n) {
        // only binary strings of the form "0", "1", "11", "111", ... are monobit
        // they are all of the form 2^k - 1 for k >= 0
        int count = 0;
        for (int i = 0; i <= n; ++i) {
            if (isMonobit(i)) {
                count++;
...
private:
    bool isMonobit(int num) {
        // A number is monobit if it is 0 or if it has the form 2^k - 1
