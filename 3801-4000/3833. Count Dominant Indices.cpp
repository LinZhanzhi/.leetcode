...
public:
    int dominantIndices(vector<int>& nums) {
        int n = nums.size();
        if (n == 1) return 0; // The rightmost element is not dominant

        int count = 0;
        long long suffixSum = 0; // To store the sum of elements to the right
        for (int i = n - 1; i >= 0; --i) {
            if (i < n - 1) { // Check if the current element is dominant
                double average = static_cast<double>(suffixSum) / (n - 1 - i);
                if (nums[i] > average) {
                    count++;
                }
            }
            suffixSum += nums[i]; // Update the suffix sum
        }
        return count;
    }
};